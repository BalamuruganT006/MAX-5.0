...
C_BG       = "#030012"
C_PRI      = "#00f0ff"   # neon cyan
C_PRI_DIM  = "#005577"
C_MID      = "#7b2fff"   # electric purple
C_DIM      = "#1a0a3e"
C_DIMMER   = "#0a0420"
C_ACC      = "#ff00aa"   # hot magenta
C_ACC2     = "#ffcc00"   # amber
C_TEXT     = "#b0f0ff"
C_PANEL    = "#06001a"
C_GREEN    = "#00ff88"
...
class MAXUI:
    ...
    def _init_particles(self):
        ...
        for _ in range(40):
            self.particles.append({
                ...
                "c":  random.choice([C_PRI, C_MID, C_ACC, C_GREEN]),
            })
    ...
    def _init_data_streams(self):
        ...
        for i in range(6):
            self.data_streams.append({
                ...
                "len":  random.randint(12, 30),
                "a":    random.uniform(15, 50),
                "chars": [random.choice("01アイウエオカキクケコ") for _ in range(30)],
            })
    ...
    def _animate(self):
        ...
        for p in self.particles:
            p["x"] += p["dx"]
            p["y"] += p["dy"]
            p["a"] += random.uniform(-3, 3)
            p["a"] = max(20, min(140, p["a"]))
            if p["y"] < -10:
                p["y"] = self.H + 10
        ...
        for ds in self.data_streams:
            ds["y"] += ds["speed"]
            if ds["y"] > self.H + ds["len"] * 14:
                ds["y"] = -ds["len"] * 14
                ds["x"] = random.uniform(40, self.W - 40)
                ds["chars"] = [random.choice("01アイウエオカキクケコ⟨⟩∆∇ΣΩ") for _ in range(30)]
    ...
    def _draw(self):
        ...
        # ── Deep space gradient (simulated with layered rects) ────────────────
        for y in range(0, H, 60):
            frac = y / H
            r = int(3 * (1 - frac))
            g = int(0)
            b = int(18 + 12 * frac)
            c.create_rectangle(0, y, W, y + 60,
                               fill=f"#{r:02x}{g:02x}{b:02x}", outline="")

        # ── Hexagonal grid ───────────────────────────────────────────────────
        hex_sz = 28
        hex_h  = hex_sz * math.sqrt(3)
        hex_a  = 18
        hex_col = self._ac(0, 240, 255, hex_a)
        for row in range(-1, int(H / hex_h) + 2):
            for col in range(-1, int(W / (hex_sz * 1.5)) + 2):
                hx = col * hex_sz * 1.5
                hy = row * hex_h + (hex_h / 2 if col % 2 else 0)
                pts = []
                for i in range(6):
                    angle = math.radians(60 * i - 30)
                    px = hx + (hex_sz * 0.5) * math.cos(angle)
                    py = hy + (hex_sz * 0.5) * math.sin(angle)
                    pts.extend([px, py])
                c.create_polygon(*pts, outline=hex_col, fill="", width=0.5)

        # ── Data streams (Matrix-style falling characters) ────────────────────
        for ds in self.data_streams:
            for j in range(ds["len"]):
                cy = ds["y"] - j * 14
                if cy < 0 or cy > H:
                    continue
                char_a = int(ds["a"] * (1.0 - j / ds["len"]))
                char_col = self._ac(0, 240, 255, char_a) if not self.muted else self._ac(255, 50, 100, char_a)
                c.create_text(ds["x"], cy, text=ds["chars"][j % len(ds["chars"])],
                              fill=char_col, font=("Courier", 9))
        ...
        for y in range(0, H, 3):
            c.create_line(0, y, W, y, fill=self._ac(0, 240, 255, scan_a), width=1)
        ...
        # ── Dönen halkalar (holographic rings) ───────────────────────────────
        ...
        # ── Extra holographic ring (purple accent) ───────────────────────────
        holo_r = int(FW * 0.55)
        holo_a = max(0, min(255, int(self.halo_a * 0.3)))
        holo_col = self._ac(123, 47, 255, holo_a)
        for s in range(8):
            start = (self.scan_angle * 0.5 + s * 45) % 360
            c.create_arc(FCX-holo_r, FCY-holo_r, FCX+holo_r, FCY+holo_r,
                         start=start, extent=20,
                         outline=holo_col, width=1, style="arc")
        ...
        # ── Header ────────────────────────────────────────────────────────────
        HDR = 62
        c.create_rectangle(0, 0, W, HDR, fill="#030018", outline="")
        c.create_line(0, HDR, W, HDR, fill=C_MID, width=1)

        # Neon glow line under header
        c.create_line(0, HDR + 1, W, HDR + 1,
                      fill=self._ac(0, 240, 255, 30), width=3)
        ...
        # Model badge with border
        c.create_text(16, 31, text=MODEL_BADGE,
                      fill=C_PRI, font=("Courier", 9, "bold"), anchor="w")
        ...
        # System readout left
        cpu_val = random.randint(12, 38)
        mem_val = random.randint(40, 72)
        c.create_text(16, 50, text=f"CPU {cpu_val}%  MEM {mem_val}%",
                      fill=C_DIM, font=("Courier", 7), anchor="w")

        # Network readout right
        net_val = random.uniform(1.2, 15.0)
        c.create_text(W - 16, 50, text=f"NET {net_val:.1f} Mbps",
                      fill=C_DIM, font=("Courier", 7), anchor="e")
        ...
        if self.muted:
            ...
            stat = "● SPEAKING"
            sc   = C_ACC
        elif self.max_state == "THINKING":
            sym  = "◈" if self.status_blink else "◇"
            stat = f"{sym} THINKING"
            sc   = C_ACC2
        elif self.max_state == "PROCESSING":
            sym  = "▷" if self.status_blink else "▶"
            stat = f"{sym} PROCESSING"
            sc   = C_ACC2
        elif self.max_state == "LISTENING":
            sym  = "●" if self.status_blink else "○"
        ...
        # ── Ses dalgası (enhanced frequency visualizer) ──────────────────────
        wy = sy + 22
        N  = 48
        BH = 22
        bw = 5
        ...
        for i in range(N):
            if self.muted:
                ...
                # Gradient from cyan to purple based on height
                frac = hb / BH
                if frac > 0.6:
                    col = C_ACC
                elif frac > 0.3:
        ...
        # ── Footer ────────────────────────────────────────────────────────────
        c.create_rectangle(0, H - 28, W, H, fill="#030018", outline="")
        c.create_line(0, H - 28, W, H - 28, fill=C_MID, width=1)
        c.create_line(0, H - 29, W, H - 29,
                      fill=self._ac(123, 47, 255, 20), width=2)
        ...
        c.create_text(W // 2, H - 14, fill=C_DIM, font=("Courier", 8),
                      text="FatihMakes Industries  ·  CLASSIFIED  ·  MAX V5")

        # Uptime counter (left)
        uptime_h = (t // 3600) % 24
        uptime_m = (t // 60) % 60
        c.create_text(16, H - 14, fill=C_DIM, font=("Courier", 7),
                      text=f"UPTIME {uptime_h:02d}:{uptime_m:02d}", anchor="w")
    ...
    def write_log(self, text: str):
        ...
        elif tl.startswith("max:") or tl.startswith("ai:"):
    ...
    def _show_setup_ui(self):
        ...
        tk.Label(self.setup_frame, text="◈  INITIALISATION REQUIRED",
                 fg=C_PRI, bg="#030018", font=("Courier", 13, "bold")).pack(pady=(18, 4))
        tk.Label(self.setup_frame,
                 text="Enter your Gemini API key to boot MAX.",
                 fg=C_MID, bg="#030018", font=("Courier", 9)).pack(pady=(0, 10))

        tk.Label(self.setup_frame, text="GEMINI API KEY",
                 fg=C_DIM, bg="#030018", font=("Courier", 9)).pack(pady=(8, 2))
        self.gemini_entry = tk.Entry(
            self.setup_frame, width=52, fg=C_TEXT, bg="#050020",
            insertbackground=C_PRI, borderwidth=0, font=("Courier", 10), show="*"
        )
